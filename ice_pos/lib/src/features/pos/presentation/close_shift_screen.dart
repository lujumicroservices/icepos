...
class _CloseShiftScreenState extends ConsumerState<CloseShiftScreen> {
  ...
  Future<void> _loadCurrentShift() async {
    ...
    if (shift == null && mounted) {
      ...
      shift = await ref.read(posRepositoryProvider).getCurrentShift();
    }
    if (!mounted) return;
    ...
    setState(() {
      ...
      _error = null;
      if (totals != null) {
        _debitController.text = totals.debitSales > 0 ? totals.debitSales.toStringAsFixed(2) : '';
        _creditController.text = totals.creditSales > 0 ? totals.creditSales.toStringAsFixed(2) : '';
      }
    });
  }
  ...
  Future<void> _submitToSummary() async {
    ...
    final cash = double.tryParse(_cashController.text.trim().replaceFirst(',', '.'));
    if (cash == null || cash < 0) {
      setState(() => _error = l10n.validAmount);
      return;
    }
    final debit = double.tryParse(_debitController.text.trim().replaceFirst(',', '.')) ?? 0;
    final credit = double.tryParse(_creditController.text.trim().replaceFirst(',', '.')) ?? 0;
    if (debit < 0 || credit < 0) {
      setState(() => _error = l10n.validAmount);
      return;
    }
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _SummaryRow(label: 'Gastos / retiros', value: '-\$${expenses.toStringAsFixed(2)}'),
          ...
          Container(
            ...
            child: Center(
              child: Text(
                isBalanced
                    ? 'Cuadre correcto'
                    : 'Diferencia: \$${difference.abs().toStringAsFixed(2)}',
                style: GoogleFonts.inter(
                  fontSize: 20,
                  fontWeight: FontWeight.bold,
